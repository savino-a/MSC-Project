...
class Qiskit_Problem:
    ...
    def _simu_acc_decc_constraint_linear(self, Nc):
        ...
        for i in range(0, Nc):
            self.mip.add_constraint(0 >= self.x[i] + self.y[i] - 1, f"z_u_p_d_{i}")
    ...
    def _solve_v2_(self):
        meo = MinimumEigenOptimizer(
            QAOA(reps=self.p, sampler=Sampler(), optimizer=COBYLA(maxiter=100))
        )
        Nc = self.N
        result_qaoa = meo.solve(self.qubo)
        x_value = []
        y_value = []
        for i in range(0, Nc):
            x_value.append(result_qaoa[0 + i])
            y_value.append(result_qaoa[Nc + i])
        time = np.arange(Nc + 1)
        distn, velo = distance(1, x_value, y_value, Nc)
...
pb = Qiskit_Problem(N=4, D=1, vmax=1, eff=False, p=1, dist_tolerance=0)
pb._solve_v2_()
